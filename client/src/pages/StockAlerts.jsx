...
const StockAlerts = () => {
  ...
  const fetchStockAlerts = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/stock-alerts/?page=${page}&limit=${limit}`
      );
      if (!response.ok) {
  ...
  const renderAlertMessage = (alert) => {
    ...
    return (
      <Card
        sx={{
          mb: 2,
          borderLeft: `5px solid ${getAlertIcon(alertType).props.sx.color}`,
        }}
      >
        <CardContent>
          <Typography
  ...
  const handleCheckInventory = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5001/api/inventory/check");
      if (!response.ok) {
      ...
      setTimeout(async () => {
        toast.success(
          data.message || "Inventory check completed successfully!",
          { autoClose: 3000 }
        );
