...
router.post("/signup", async (req, res) => {
  ...
  if (!password || !validatePassword(password))
    errors.push(
      "Password must be at least 6 characters long, contain a number, an uppercase letter, and a special character"
    );
  if (!role || !["manager", "admin"].includes(role))
    errors.push('Role must be either "manager" or "admin"');
